...
Connection.prototype.connect = async function() {
  ...
  if (config.proxy) {
    const options = {
      ...
      destination: {
        host: config.host, // ipv4, ipv6, or hostname
        port: config.port
      }
    }
...
Connection.prototype._login = function() {
  ...
  var reentry = function(err) {
    ...
    if (self._config.id) {
      self._enqueue(`ID ("name" "${escape(self._config.id.name)}" "version" "${escape(self._config.id.version)}" "vendor" "${escape(self._config.id.vendor)}")`, reentry);
    }
